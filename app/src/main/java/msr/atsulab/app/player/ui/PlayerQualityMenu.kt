...
internal class PlayerQualityMenu(
    private val activity: Activity,
    private val callbacks: Callbacks
) {

    interface Callbacks {
        fun videoQualities(): List<VideoQuality>
        fun selectedQualityTrackId(): String?
        fun onQualitySelected(trackId: String?)
        fun onQualityMenuDismissed()
    }
    ...
    fun show() {
        ...
        PlayerQualityMenuModel.options(
            qualities = callbacks.videoQualities(),
            selectedTrackId = callbacks.selectedQualityTrackId()
        ).forEach { option ->
            panel.addView(
                qualityRow(option, density) {
                    callbacks.onQualitySelected(option.id)
                    dismiss()
                },
        ...
        root.addView(
            ...
            FrameLayout.LayoutParams(
                dp(PlayerShellMetrics.QUALITY_PANEL_WIDTH_DP, density),
        ...
        val shownPopup = PopupWindow().apply {
            ...
            setOnDismissListener {
                popup = null
                callbacks.onQualityMenuDismissed()
            }
        }
        popup = shownPopup

        root.alpha = 0f
        scroll.translationX = dp(PlayerShellMetrics.QUALITY_PANEL_WIDTH_DP, density).toFloat()
    ...
    private fun qualityRow(option: VideoQualityOption, density: Float, onClick: () -> Unit): TextView {
