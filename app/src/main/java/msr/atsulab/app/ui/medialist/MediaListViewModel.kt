...
import msr.atsulab.app.helper.pojo.MediaListAdapterComponent
...
class MediaListViewModel(
    ...
    fun updateProgress(mediaList: MediaList, newProgress: Int, isProgressVolume: Boolean) {
        val currentProgress = when (mediaList.media.type) {
            msr.atsulab.app.type.MediaType.ANIME -> mediaList.progress
            msr.atsulab.app.type.MediaType.MANGA -> if (isProgressVolume) (mediaList.progressVolumes ?: 0) else mediaList.progress
            else -> 0
        }
        ...
        val maxProgress = when (mediaList.media.type) {
            msr.atsulab.app.type.MediaType.ANIME -> mediaList.media.episodes
            msr.atsulab.app.type.MediaType.MANGA -> if (isProgressVolume) mediaList.media.volumes else mediaList.media.chapters
            else -> null
        }
