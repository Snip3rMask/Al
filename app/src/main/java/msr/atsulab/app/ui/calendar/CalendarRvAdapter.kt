...
import msr.atsulab.app.data.response.anilist.AiringSchedule
import msr.atsulab.app.data.response.anilist.Media
import msr.atsulab.app.data.response.anilist.MediaExternalLink
import msr.atsulab.app.databinding.ListCalendarBinding
    ...
    inner class ItemViewHolder(private val binding: ListCalendarBinding) : ViewHolder(binding) {
        override fun bind(item: AiringSchedule, index: Int) {
            with(binding) {
                ...
                calendarMediaListStatusText.text = item.media.mediaListEntry?.status?.getString(item.media.type?.getMediaType() ?: msr.atsulab.app.helper.enums.MediaType.ANIME)?.uppercase()
